...
if __name__ == "__main__":
    ...
    MNIST_BlArch_instance = MNIST_BlArch(
        input_shape=(28, 28, 1),
        batch_size=512,
        test_batch_size=32,
        optimizer="adam",
    ...
    feat_extract_block_instance = FeatureExtractionBlock(
        input_shape=(28, 28, 1), parent_block=MNIST_BlArch_instance, args=None
    )  # outputshape = (28,28,67)
    conv_2d_instance = Layer(
        input_shape=(28, 28, 1), parent_block=feat_extract_block_instance, args=args
    )  # core.layer.layershape (28,28,67)

    method_names = [
        name
        for name in dir(conv_2d_instance)
        if callable(getattr(conv_2d_instance, name))
    ]
