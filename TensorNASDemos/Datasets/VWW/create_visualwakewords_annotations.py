...
def create_visual_wakeword_annotations(
        ...
        for ann_id in coco.getAnnIds(imgIds=img_id, catIds=foreground_class_id):
            ...
            if "area" in ann:
                ...
                if normalized_ann_area > object_area_threshold:
                    ...
                    new_ann_id += 1
    print(
        "There are {} images that now have label {}, of the {} images in total.".format(
            len(positive_img_ids), foreground_class_name, len(coco.imgs)
        )
    )
    ...
    with open(visualwakewords_annotations_path, "w") as fp:
        print("Dumping to JSON, this can take a while")
        json.dump(
            {
                "info": info,
                "licenses": licenses,
...
def main(args):
    ...
    train_annotations_file = os.path.realpath(
        os.path.expanduser(args.train_annotations_file)
    )
    val_annotations_file = os.path.realpath(
        os.path.expanduser(args.val_annotations_file)
    )
    visualwakewords_annotations_train = os.path.join(output_dir, "instances_train.json")
    visualwakewords_annotations_val = os.path.join(output_dir, "instances_val.json")
