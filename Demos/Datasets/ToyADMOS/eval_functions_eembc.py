    ...
    for class_item in range(n_classes):
        ...
        for threshold_item in range(len(thresholds) - 1):
            roc_auc[class_item] += (
                0.5
                * (
                    tpr[class_item, threshold_item]
                    + tpr[class_item, threshold_item + 1]
                )
                * (
                    fpr[class_item, threshold_item]
                    - fpr[class_item, threshold_item + 1]
                )
            )
...
def calculate_ae_accuracy(y_pred, y_true):
    thresholds = np.amin(y_pred) + np.arange(0.0, 1.0, 0.01) * (
        np.amax(y_pred) - np.amin(y_pred)
    )
    accuracy = 0
...
def calculate_ae_auc(y_pred, y_true, name):
    # initialize all arrays
    thresholds = np.amin(y_pred) + np.arange(0.0, 1.01, 0.01) * (
        np.amax(y_pred) - np.amin(y_pred)
    )
    roc_auc = 0
    ...
    for threshold_item in range(len(thresholds) - 1):
        roc_auc += (
            0.5
            * (tpr[threshold_item] + tpr[threshold_item + 1])
