...
# taken directly from https://github.com/mlcommons/tiny/blob/master/v0.5/training/keyword_spotting/get_dataset.py
...
import Demos.Datasets.SpeechCommands.kws_util as kws_util
import Demos.Datasets.SpeechCommands.keras_model as models
...
def get_preprocess_audio_func(model_settings, is_training=False, background_data=[]):
    def prepare_processing_graph(next_element):
        ...
        if is_training and background_data != []:
            background_volume_range = tf.constant(
                background_volume_range_, dtype=tf.float32
            )
        ...
        if model_settings["feature_type"] == "mfcc":
            ...
            lower_edge_hertz, upper_edge_hertz = (
                0.0,
                model_settings["sample_rate"] / 2.0,
            )
            linear_to_mel_weight_matrix = tf.signal.linear_to_mel_weight_matrix(
...
def get_training_data(Flags, get_waves=False, val_cal_subset=False):
    ...
    (ds_train, ds_test, ds_val), ds_info = tfds.load(
        "speech_commands", split=splits, data_dir=Flags.data_dir, with_info=True
    )

    if (
        val_cal_subset
    ):  # only return the subset of val set used for quantization calibration
        ...
        for d in ds_val:
            if count in cal_indices:  # this is one of the calibration inpus
                ...
                if (
                    len(new_audio) < 16000
                ):  # from_tensor_slices doesn't work for ragged tensors, so pad to 16k
                    new_audio = np.pad(
                        new_audio, (0, 16000 - len(new_audio)), "constant"
                    )
        ...
        ds_val = tf.data.Dataset.from_tensor_slices(
            {"audio": val_sub_audio, "label": val_sub_labels}
        )
